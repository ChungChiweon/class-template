...
if (window.LoreAXTenant?.guardCourseAccess?.(getCurrentLessonId(), { homeUrl: "../../../index.html" })?.blocked) {
...
function getRequiredStatus(data) {
  const report = data.report;
  return [
    { label: "탐구 주제", complete: hasAny(report.title) },
    { label: "주제 선택 이유", complete: hasAny(report.topicReason) },
    { label: "탐구 질문", complete: hasAny(report.researchQuestion) },
    { label: "자료 출처·요약", complete: hasAny(report.methodology.dataSource, report.dataSummary) },
    {
      label: "서론",
      complete: hasAny(report.introduction.background, report.introduction.purpose, report.introduction.necessity),
    },
    ...
    { label: "분석 결과", complete: hasAny(report.results.summary, report.results.interpretation) },
    { label: "그래프", complete: Boolean(data.chartImage) },
    { label: "결론", complete: hasAny(report.conclusion.answer) },
    { label: "학문적 의미·한계", complete: hasAny(report.conclusion.significance, report.conclusion.limitations) },
    { label: "참고문헌", complete: normalizeList(report.references).length > 0 },
    { label: "AI 활용 기록", complete: normalizeList(report.aiUsageLogs).length > 0 },
  ];
}
...
function renderPreview(data) {
  const report = data.report;
  const student = data.student;
  const title = text(report.title) || "주제탐구보고서";
  const introBody = [
    section("연구 배경", paragraphHtml(report.introduction.background)),
    section("연구 목적", paragraphHtml(report.introduction.purpose)),
    section("연구 필요성", paragraphHtml(report.introduction.necessity)),
  ...
  const methodRows = [
    ...
    ["자료 수집 방법", report.methodology.collectionMethod],
    ["자료 분석 방법", report.methodology.analysisMethod],
  ].filter(([, value]) => text(value));
  const resultMeta = [
    ["그래프 제목", report.results.chartTitle],
    ["X축 설명", report.results.xAxisDescription],
  ...
  const sentenceCompare = [
    section("자기 초안", paragraphHtml(report.selfDraft)),
    section("AI 수정 문장", paragraphHtml(report.aiRevisedSentence)),
    section("최종 수정 문장", paragraphHtml(report.finalRevisedSentence)),
  ].join("");
  const contactVisible = Boolean(LOREAX_CONTACT.kakaoChannelUrl || LOREAX_CONTACT.inquiryUrl || LOREAX_CONTACT.qrImageUrl);

  dom.reportPreview.innerHTML = `
    <section class="cover-page avoid-break">
      <p class="document-type">주제탐구보고서</p>
      <h1>${escapeHtml(title)}</h1>
      <dl>
        ${text(student.name) ? `<div><dt>학생 이름</dt><dd>${escapeHtml(student.name)}</dd></div>` : ""}
        ${text(student.studentNumber) ? `<div><dt>학번</dt><dd>${escapeHtml(student.studentNumber)}</dd></div>` : ""}
    ...
    ${section("1. 탐구 주제와 선택 이유", `${paragraphHtml(report.title)}${paragraphHtml(report.topicReason)}`)}
    ${section("2. 탐구 질문", paragraphHtml(report.researchQuestion))}
    ${section("3. 연구 범위", paragraphHtml(report.researchScope))}
    ${section("4. 자료 출처 및 자료 요약", `${paragraphHtml(report.methodology.dataSource)}${paragraphHtml(report.dataSummary)}`)}
    ${section("5. 연구 배경 및 목적", introBody, "nested-section")}
    ${section(
      "6. 연구 방법",
      `${methodRows.length ? `<table class="method-table"><tbody>${methodRows
        .map(([label, value]) => `<tr><th>${escapeHtml(label)}</th><td>${escapeHtml(value)}</td></tr>`)
        .join("")}</tbody></table>` : ""}${paragraphHtml(report.cleanedDataNotes)}`
    )}
    ${section("7. 자료 분석 결과", `${paragraphHtml(report.results.summary)}${resultMeta.length ? listHtml(resultMeta.map(([label, value]) => `${label}: ${value}`)) : ""}`)}
    ${
      data.chartImage
        ? section(
            "8. 시각화 그래프",
    ...
    ${section("9. 그래프 해석", paragraphHtml(report.results.interpretation))}
    ${section("10. 자기 문장과 AI 문장 비교", sentenceCompare, "nested-section")}
    ${section("11. 결론", paragraphHtml(report.conclusion.answer))}
    ${section("12. 학문적 의미", paragraphHtml(report.conclusion.significance))}
    ${section(
      "13. 연구의 한계 및 향후 탐구",
      `${paragraphHtml(report.conclusion.limitations)}${paragraphHtml(report.conclusion.futureResearch)}`
    )}
    ${section("14. 참고문헌", listHtml(report.references))}
    ${section("15. AI 활용 기록", `${listHtml(report.aiUsageLogs)}${paragraphHtml(report.aiFinalRevisionLog)}`)}
    ${section(
      "16. 동료 피드백 및 반영 내용",
      `${paragraphHtml(report.peerFeedback)}${paragraphHtml(report.feedbackReflection)}`
    )}
