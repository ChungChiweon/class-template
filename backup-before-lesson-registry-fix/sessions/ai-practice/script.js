const hotelPromoLesson = {
  ...
  periods: [
    {
      period: "1교시",
      title: "HeyGen 캐릭터 영상 체험",
      goal: "AI 캐릭터 생성과 짧은 나레이션 영상 제작 흐름을 이해합니다.",
      activity: "호텔 안내 캐릭터를 생성하고, 나레이션을 입력한 뒤 짧은 영상 생성까지 체험합니다.",
      studentTasks: ["호텔 콘셉트 정하기", "호텔 안내 캐릭터 만들기", "짧은 소개 문장 입력하기", "생성 결과 확인하기"],
      teacherTasks: ["무료버전 다운로드 제한 안내", "캐릭터 일관성의 중요성 설명", "생성 버튼 클릭 전 확인 순서 안내"],
      tools: "HeyGen",
      toolKeys: ["heygen"],
      output: "AI 호텔 안내 캐릭터 1명, 나레이션 입력, 체험용 영상 생성",
      outputs: ["AI 호텔 안내 캐릭터 1명", "나레이션 입력 결과", "체험용 영상 생성 화면"],
      checkpoints: ["호텔 직원처럼 보이는가", "나레이션이 너무 길지 않은가", "무료 크레딧을 낭비하지 않았는가"],
...
필수 장면 구조: 인트로 1장면 + 장소소개 2장면 + 아웃트로 1장면
...
function applyFullPromptWorkflow() {
  Object.values(lessons).forEach((lesson) => {
    const hasFullPromptTab = lesson.promptTabs?.some((tab) => tab.tabId === "full-prompt-set");
    if (!hasFullPromptTab) {
      lesson.promptTabs.unshift(createFullPromptGenerationTab(lesson));
    }
  });

  hotelPromoLesson.periods[0] = {
    ...hotelPromoLesson.periods[0],
...
const practiceSteps = [
  {
    title: "HeyGen 캐릭터 생성",
    toolKey: "heygen",
    role: "호텔 안내 캐릭터를 만들고 나레이션을 입력해 짧은 영상을 생성합니다.",
    activity: "호텔 콘셉트에 맞는 안내 캐릭터를 만든 뒤, 소개 나레이션을 입력하고 영상 생성까지 체험합니다.",
    output: "AI 호텔 안내 캐릭터 1명, 나레이션 입력, 체험용 영상 생성",
    checklist: ["호텔 콘셉트 정하기", "캐릭터 생성 완료", "나레이션 입력", "영상 생성 체험"],
    guides: ["screen", "style", "appearance"],
    detailTitle: "HeyGen 1회 체험 진행 순서",
    details: [
      {
        title: "캐릭터 생성",
        text: "호텔을 대표할 안내 캐릭터의 이름, 나이대, 성별 느낌, 외형 계열, 스타일, 복장 분위기를 입력합니다. 호텔 프런트 직원처럼 단정하고 신뢰감 있게 보이도록 설정합니다.",
      },
      {
        title: "나레이션 입력",
        text: "학생이 만든 10초 호텔 소개 문장을 HeyGen에 입력합니다. 문장은 짧고 또렷하게 작성하고, 호텔 첫인사 또는 객실/서비스 소개 중 하나를 선택해 넣습니다.",
      },
      {
        title: "영상 생성 체험",
        text: "캐릭터가 나레이션을 말하는 짧은 소개 영상을 생성합니다. 무료버전에서는 다운로드가 제한될 수 있으므로, 수업에서는 생성 결과를 화면에서 확인하는 1회 체험으로 진행합니다.",
      },
      {
        title: "확인 포인트",
        text: "캐릭터 얼굴, 의상, 말투, 호텔 직원 느낌이 처음 설정과 일치하는지 확인합니다. 이후 GPT 기준 이미지와 Google Flow 영상에서도 같은 캐릭터로 유지될 수 있도록 특징을 기록합니다.",
      },
    ],
  },
  {
...
function renderLessonMetadata() {
  ...
  heroRecommendedScenes.textContent = `${metadata.recommendedSceneCount || lessonData.creditRules?.recommendedScenes || 4}장면`;
...
function renderLesson() {
  ...
  creditTeacherNotes.hidden = !isTeacherMode();
}
  ...
  practiceAccordion.innerHTML = activePracticeSteps
    .map((step, index) => {
      ...
      return `
        ...
        ${isHotelLesson() && index === 3 ? renderScenePromptBoard() : ""}
      `;
    })
    .join("");
}
...
function getPromptGeneratorDefaults() {
  ...
  const defaultMap = {
    hotelPromo: {
      projectTitle: "AI 호텔 홍보영상 제작",
      placeName: "오션뷰 프리미엄 호텔",
      placeOne: "호텔 로비",
      placeTwo: "객실",
      characterRole: "친절하고 전문적인 호텔리어",
      tone: "고급스럽고 따뜻한 호텔 홍보영상",
    },
    dataAnalysisReport: {
  ...
  return {
    ...
    projectTitle: metadata.classTitle || defaultMap[currentLessonId]?.projectTitle || defaultMap.hotelPromo.projectTitle,
  };
}
...
HeyGen 체험용 대사 초안:
"안녕하세요. ${values.placeName}에 오신 것을 환영합니다. 오늘은 ${values.placeOne}와 ${values.placeTwo}를 짧게 소개해드리겠습니다."`;
...
주제/장소/제품명: ${values.placeName}
캐릭터 역할: ${values.characterRole}
소개할 장소 1: ${values.placeOne}
소개할 장소 2: ${values.placeTwo}
영상 분위기: ${values.tone}
권장 구성: 인트로 1 + 장소소개 2 + 아웃트로 1, 총 4장면
...
- HeyGen 합본에서 나온 나레이션은 인트로, 장소소개 1, 장소소개 2, 아웃트로 순서로 각각 하나씩 배분합니다.
...
[promptProjectTitle, promptPlaceName, promptPlaceOne, promptPlaceTwo, promptCharacterRole, promptTone, promptHeygenResult].forEach((control) => {
