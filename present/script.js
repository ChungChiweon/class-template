...
const lessonRuntimeData = window.LoreAXPresenterRegistry || window.LoreAXLessonRuntimeData || {};
const presenterSessionStartedAt = Date.now() - 5000;
...
function getActiveLessonId() {
  ...
  const lessonId = queryLessonId || storedLessonId || DEFAULT_LESSON_ID;
  const resolvedLessonId = lessonRuntimeData[lessonId] ? lessonId : DEFAULT_LESSON_ID;
  try {
...
const activeLesson = lessonRuntimeData[activeLessonId] || { id: activeLessonId, title: "AI 실습형 수업", periods: [] };
const useMockPresence = new URLSearchParams(window.location.search).get("mock") === "1";
const lessonSteps = (activeLesson.periods || []).map((period, index) => ({
  period: activeLessonId === "dataAnalysisReport" ? period.title?.split("·")[0].trim() || `${index + 1}단계` : `${index + 1}교시`,
...
const researchReportMockStudents = [
  { id: "r01", name: "김민서", step: "1차시-1 · 탐구 주제 설정", requiredInputs: 5, completedInputs: 4, checklistTotal: 5, checklistDone: 4, lastSeenOffset: 12000, pdfGenerated: false },
  { id: "r02", name: "이도윤", step: "1차시-2 · 탐구 질문 정교화", requiredInputs: 8, completedInputs: 5, checklistTotal: 5, checklistDone: 3, lastSeenOffset: 28000, pdfGenerated: false },
  { id: "r03", name: "박서연", step: "2차시 · 자료 분석과 시각화", requiredInputs: 10, completedInputs: 8, checklistTotal: 6, checklistDone: 5, lastSeenOffset: 73000, pdfGenerated: false },
  { id: "r04", name: "최지우", step: "3차시 · 보고서 초안 작성", requiredInputs: 9, completedInputs: 9, checklistTotal: 7, checklistDone: 6, lastSeenOffset: 18000, pdfGenerated: false },
  { id: "r05", name: "정하준", step: "4차시 · 보고서 완성", requiredInputs: 10, completedInputs: 10, checklistTotal: 9, checklistDone: 9, lastSeenOffset: 95000, pdfGenerated: true },
];
...
const dom = {
  ...
  topicResearchQrCard: document.querySelector("#topicResearchQrCard"),
  openQrModalButton: document.querySelector("#openQrModalButton"),
  closeQrModalButton: document.querySelector("#closeQrModalButton"),
  qrModal: document.querySelector("#qrModal"),
  authStatus: document.querySelector("#presentAuthStatus"),
  kakaoLoginButton: document.querySelector("#presentKakaoLoginButton"),
  kakaoLogoutButton: document.querySelector("#presentKakaoLogoutButton"),
...
let presenceEmptyMessage = "아직 접속한 학생이 없습니다.";

if (dom.topicResearchQrCard) {
  dom.topicResearchQrCard.hidden = activeLessonId !== "dataAnalysisReport";
}
...
function loadLocalPresence() {
  ...
  for (let index = 0; index < localStorage.length; index += 1) {
    ...
    try {
      ...
      if (value?.lessonId === activeLessonId || (activeLessonId === DEFAULT_LESSON_ID && !value?.lessonId)) rows.push(value);
    } catch {
...
function getStudents() {
  const now = Date.now();
  const sourceMockStudents = useMockPresence
    ? activeLessonId === "dataAnalysisReport"
      ? researchReportMockStudents
      : mockStudents
    : [];
  const mockRows = sourceMockStudents.map((student) => ({
    ...student,
...
function filterCurrentPresenterSession(students) {
  if (useMockPresence) return students;
  return students.filter((student) => {
    const normalized = normalizeStudent(student);
    return normalized.lastSeenAt >= presenterSessionStartedAt;
  });
}
...
function renderConnectionStatus(mode = presenceMode) {
  ...
  dom.presenceConnectionStatus.textContent = labelMap[mode] || "서버 연결 확인 중";
...
function renderPresence(studentsSource = presenceRows.length ? presenceRows : getStudents()) {
  const students = sortStudents(filterCurrentPresenterSession(studentsSource).map((student) => normalizeStudent(student)));
  ...
  dom.studentRows.innerHTML = visibleStudents
    ...
    .join("") || `<tr><td colspan="6" class="empty-row">${escapeHtml(presenceEmptyMessage)}</td></tr>`;
}
...
function openQrModal() {
  ...
  dom.closeQrModalButton?.focus();
}

function closeQrModal() {
  if (!dom.qrModal) return;
  dom.qrModal.hidden = true;
  dom.openQrModalButton?.focus();
}

dom.openQrModalButton?.addEventListener("click", openQrModal);
dom.closeQrModalButton?.addEventListener("click", closeQrModal);
dom.qrModal?.addEventListener("click", (event) => {
  if (event.target.closest("[data-close-qr-modal]")) closeQrModal();
});
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && !dom.qrModal?.hidden) closeQrModal();
});
...
async function refreshSupabasePresence() {
  if (!window.LoreAXSupabase?.isEnabled?.()) {
    ...
    presenceEmptyMessage = useMockPresence ? "mock 학생 데이터가 없습니다." : "Supabase가 연결되지 않았습니다. 학생 화면의 로컬 현황만 표시합니다.";
    renderConnectionStatus(useMockPresence ? "mock" : "local");
  ...
  if (result.mode === "supabase") {
    ...
    presenceRows = getStudents();
    presenceEmptyMessage = "서버 연결 실패 - 학생 현황을 불러올 수 없습니다.";
    renderConnectionStatus(useMockPresence ? "mock" : "offline");
  }
  renderPresence(presenceRows);
}
...
function getAuthRedirectTo() {
  return `${window.location.origin}/present/?lesson=${encodeURIComponent(activeLessonId)}`;
}
...
async function renderAuthState() {
  ...
  if (!window.LoreAXSupabase?.isEnabled?.()) {
    dom.authStatus.textContent = "Supabase 미연결";
  ...
  if (user) {
    ...
    dom.authStatus.textContent = "로그인 없이 현황 확인";
    if (dom.kakaoLoginButton) {
      dom.kakaoLoginButton.hidden = false;
      dom.kakaoLoginButton.disabled = false;
    }
...
function initKakaoAuth() {
  ...
  dom.kakaoLoginButton?.addEventListener("click", async () => {
    if (!window.LoreAXSupabase?.isEnabled?.()) {
      dom.authStatus.textContent = "Supabase 설정 후 카카오 로그인을 사용할 수 있습니다";
      return;
    }
    ...
    if (!result.ok) {
      dom.kakaoLoginButton.disabled = false;
      dom.authStatus.textContent = result.message || "카카오 로그인 연결을 확인하세요";
    }
  });

  dom.kakaoLogoutButton?.addEventListener("click", async () => {
    const result = await window.LoreAXSupabase?.signOut?.();
    if (!result?.ok) dom.authStatus.textContent = result?.message || "로그아웃을 확인하세요";
